...
public:
    ...
    void deleteContact(const string& target)
    {

        for (ContactPtr iter = head; iter != NULL; iter = iter->next)
        {
            ContactPtr iternext = iter->next;
            if((iternext->getName() != target) && (iter->next != NULL) && (iter != NULL))
            {
                cout << "Target name was not found!" << endl;
            }
